 ...
 **/
 ...
 static const float deg2rad = 0.01745;
 ...
 static void rotateV(float* est_vec, float* angle_delta);
 ...
 */
 void attitude_estimate(ad* attitude_data, sd* sensors_data)
 {
	 ...
	 //float att_gyro[3];
	 float w_gyro2acc = 200;                 						//陀螺仪相对加计比值
	 u8 i = 0;
	 float deltaT = 0.02;								  							//这个如果能通过计算运行循环时间解算就比较好了
	 float acc_mag = 0;																	//加计模长
	 float inv_norm;
   ...
   //printf_sensors_data_estimate(*sensors_data);     //读出正确取向传感器数据分析
	 for(; i < 3; i++)
	 {
	   euler_delta[i] = sensors_data->gyro[i]*deltaT*deg2rad;   //计算相比较上次解算的旋转欧拉角
	   attitude_data->angle_rate[i] = sensors_data->gyro[i]; 
		 //printf("euler delta: %.2f\n", euler_delta[i]);
	 }
	 ...
	 {
			acc_mag = sensors_data->acc[i]*sensors_data->acc[i]+acc_mag;
	 }
	 inv_norm = fast_inv_sqrt(acc_mag);									//计算得到模方根分之一
	 for(i = 0; i<3; i++)
	 {
			sensors_data->acc[i] = sensors_data->acc[i]*inv_norm;    
	 }
	 
	 rotateV(att_est, euler_delta);											//前一次估计陀螺仪矢量迭代解算当前矢量 
	 
	 if(0.7 < acc_mag < 1.3)
		 for (i = 0; i<3; i++)
		 {
			 att_est[i] = (w_gyro2acc*att_est[i]+sensors_data->acc[i])/(1+w_gyro2acc);
		 }
 ...
 *
 */
 void rotateV(float* est_vec, float* angle_delta)
 {
	 float vec_temp[3];
	 u8 i;
	 vec_temp[0] = angle_delta[1]*est_vec[2]-angle_delta[2]*est_vec[1];
	 vec_temp[1] = angle_delta[2]*est_vec[0]-angle_delta[0]*est_vec[2];
	 vec_temp[2] = angle_delta[0]*est_vec[1]-angle_delta[1]*est_vec[0];
	 
	 for(i=0; i<3; i++)
	 {
		 est_vec[i] += vec_temp[i];
		 //printf("estimated vector now: %.2f\n", est_vec[i]);
	 }
	 normalize(est_vec);//正交化处理
 }
