...
void get_sensors_data(struct sensors_data* sd)
{
  ...
  //if(i2c_mpu6050_check_s() && i2c_ms5611_calculate_s())
	...
	{
			i2c_mpu6050_read_acc_s(sd->acc);
			i2c_mpu6050_read_gyro_s(sd->gyro);
			i2c_mpu6050_read_temp_s(sd->temp);
		
			//printf("HMC5883L数据： \n");
	    i2c_mpu6050_read_mag_s(sd->mag); 
	    //printf("magx: %d%s%d%s%d", sd->mag[0], "magy: ", sd->mag[1], "magz: ", sd->mag[2]);
			sd->press = i2c_ms5611_calculate_s();
			//printf("MS5611数据： \n");
			//printf("press: %d\n", sd->press);
	}
