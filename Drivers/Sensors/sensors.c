...
void get_sensors_data(sd* sdata, sc* calib_data)
{
	...
	static float gyro_pre[3] = {0,0,0};					//为了计算简化，只取两个相邻加权均值
	...
	for(axis=0; axis<3; axis++)
  {
	...
	for(axis=0; axis<3; axis++)
	{
		sdata->gyro[axis] = (4*sdata->gyro[axis]+gyro_pre[axis])/5;
		gyro_pre[axis] = sdata->gyro[axis];				//存储当前值以便下一次迭代
	}
	...
	for(axis=0; axis<3; axis++)
	{
		...
		gyro_filtered[axis] -= gyro_filtered[axis]/smooth_factor;
		gyro_filtered[axis] += sdata->gyro[axis]/smooth_factor;
		sdata->gyro[axis] = gyro_filtered[axis];		
	}
	*/
